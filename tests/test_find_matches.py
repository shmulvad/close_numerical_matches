...
def is_same(matches0, matches1):
    ...
    return (matches0_set == matches1_set
...
class TestFindMatches:
    ...
    def test_custom_dist(self):
        def manhatten_dist(ndarray):
            return np.sum(np.abs(ndarray), axis=1)
    ...
    def test_value_error_on_incorrect_dist_func(self):
        def my_incorrect_dist_func(ndarray):
            return 1

        arr0 = np.ones((5, 4))
        arr1 = np.ones((5, 4))
        with pytest.raises(ValueError):
            find_matches(arr0, arr1, dist=my_incorrect_dist_func)

    def test_value_error_on_incorrect_dist_str(self):
        arr0 = np.ones((5, 4))
        arr1 = np.ones((5, 4))
        with pytest.raises(ValueError):
            find_matches(arr0, arr1, dist='non-existing func')

    def test_assert_error_on_zero_tol(self):
        arr0 = np.ones((5, 4))
        arr1 = np.ones((5, 4))
        with pytest.raises(AssertionError):
            find_matches(arr0, arr1, tol=0)

    def test_assert_error_on_too_small_bucket_tol_mult(self):
        arr0 = np.ones((5, 4))
        arr1 = np.ones((5, 4))
        with pytest.raises(AssertionError):
            find_matches(arr0, arr1, bucket_tol_mult=0)

    def test_assert_error_on_non_equiv_shape(self):
        arr0 = np.ones((5, 3))
        arr1 = np.ones((5, 4))
        with pytest.raises(AssertionError):
            find_matches(arr0, arr1)

    def test_assert_error_on_incorrect_shape(self):
        arr0 = np.ones((4, 5, 2))
        arr1 = np.ones((4, 5, 2))
        with pytest.raises(AssertionError):
            find_matches(arr0, arr1)
